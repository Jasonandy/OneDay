...
* @Description：   <p> DateHelper </p>
* @Author： - Jason    
* @CreatTime：2019年1月3日 下午7:21:45   
* @Modify By：   
...
public class DateHelper extends org.apache.commons.lang3.time.DateUtils{
	...
	public static SimpleDateFormat sdfDateTime = new SimpleDateFormat(DateHelper.DATE_FORMAT_DATETIME);
	// Global SimpleDateFormat object
	public static SimpleDateFormat sdfDateOnly = new SimpleDateFormat(DateHelper.DATE_FORMAT_DATEONLY);
	 ...
	 * 星期
	 */
	final static String chineseWeek[] = { "星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"};
	...
	public static java.sql.Timestamp toSqlTimestamp(String sDate) {
		...
		if (sDate.length() != DateHelper.DATE_FORMAT_DATEONLY.length()
				&&sDate.length() != DateHelper.DATE_FORMAT_DATETIME.length()) {
			return null;
		}
		return toSqlTimestamp(sDate, 
				sDate.length() == DateHelper.DATE_FORMAT_DATEONLY.length()
				?DateHelper.DATE_FORMAT_DATEONLY
				:DateHelper.DATE_FORMAT_DATETIME);
	...
	public static java.sql.Timestamp toSqlTimestamp(String sDate, String sFmt) {
		...
		if (sFmt.equals(DateHelper.DATE_FORMAT_DATETIME)) {
			temp = sDate.replace('/', '-');
			temp = temp + ".000000000";
		} else if (sFmt.equals(DateHelper.DATE_FORMAT_DATEONLY)) {
			temp = sDate.replace('/', '-');
			temp = temp + " 00:00:00.000000000";
			// }else if( sFmt.equals (DateHelper.DATE_FORMAT_SESSION )){
			// //Format: 200009301230
			// temp =
	...
	public static String getSysDateTimeString() {
		return toString(new java.util.Date(System.currentTimeMillis()), DateHelper.sdfDateTime);
	}
	...
	public static String toSqlTimestampString2(java.sql.Timestamp dt) {
		...
		String temp = toSqlTimestampString(dt, DateHelper.DATE_FORMAT_DATETIME);
	...
	public static String toSqlTimestampString(java.sql.Timestamp dt, String sFmt) {
		...
		temp = dt.toString();
		if (sFmt.equals(DateHelper.DATE_FORMAT_DATETIME) || // "YYYY/MM/DD
				// HH24:MI:SS"
				sFmt.equals(DateHelper.DATE_FORMAT_DATEONLY)) { // YYYY/MM/DD
	...
	/**
	 * @Description: getChineseWeek 获取星期数
	 * @param week
	 * @return String
	 * @Autor: @Jason - jasonandy@hotmail.com
	 */
	public static String getChineseWeek() {
		return chineseWeek[getWeek()-1];
	}
	...
	public static String timestampToStringYMD(java.sql.Timestamp timestamp) {
		SimpleDateFormat sdf = new SimpleDateFormat(DateHelper.DATE_FORMAT_DATEONLY);
	...
	public static boolean isOverIntervalLimit(String startDateStr, String endDateStr, int interval) {
		Date startDate = null;
		Date endDate = null;
		startDate = DateHelper.parseDate(startDateStr, DateHelper.DATE_FORMAT_DATEONLY);
		endDate = DateHelper.parseDate(endDateStr, DateHelper.DATE_FORMAT_DATEONLY);
	...
	public static Date getMinTime(Date dt) {
		Date dt1 = null;
		dt1 = DateHelper.getDateByStr(DateHelper.formatDate(dt, "yyyy-MM-dd"));
		return dt1;
	}
	...
	public static List<String> getLastDays(int countDay) {
		...
		for (int i = 0; i < countDay; i++) {
			listDate.add(DateHelper.getReqDateyyyyMMdd(DateHelper.getDate(-i)));
		}
		return listDate;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(chineseWeek[getWeek()-1]);
